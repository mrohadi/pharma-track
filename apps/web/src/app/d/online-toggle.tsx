...
import { toggleDriverOnline } from '@/lib/services/driver';
...
export function OnlineToggle({ online }: { online: boolean }) {
  ...
  function handleToggle() {
    startTransition(async () => {
      await toggleDriverOnline(!online);
    });
  }
  ...
  return (
    <div className="mb-4 flex items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3">
      ...
      >
        <span
          className={`absolute top-0.5 h-6 w-6 transform rounded-full bg-white shadow transition-transform duration-200 ${
