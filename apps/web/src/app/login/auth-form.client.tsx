  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', background: PT.border, borderRadius: 12, padding: 3 }}>
        {([['login', 'Masuk'], ['signup', 'Daftar']] as [Tab, string][]).map(([t, l]) => (
          <button
            key={t}
            type="button"
            onClick={() => setTab(t)}
        ...
        {tab === 'login' ? (
          <>
            ...
            <button
              ...
              style={{ fontSize: 13, color: PT.primary, fontWeight: 700, border: 'none', background: 'none', cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}
            >
              Daftar gratis
            </button>
          </>
...
function LoginPanel({ next }: { next?: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <h2 style={{ fontSize: 22, fontWeight: 800, color: PT.text, margin: '0 0 4px', letterSpacing: '-0.4px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
          {LOGIN_ROLES.map((r) => (
            ...
            >
              ...
              <div style={{ fontSize: 12, fontWeight: 600, color: loginRole === r.id ? PT.primaryText : PT.muted }}>
                {r.label}
              </div>
            </button>
          ))}
      ...
      {loginRole === 'admin' && (
        <div style={{ padding: '10px 14px', background: PT.warningLight, borderRadius: 10, fontSize: 12.5, color: 'oklch(0.48 0.14 75)', fontWeight: 500 }}>
      ...
      <form action={onSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <AuthField label="Email" name="email" type="email" placeholder="anda@example.co.id" icon="✉️" />
        <AuthField label="Password" name="password" type="password" placeholder="••••••••" icon="🔒" />
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <a href="#" style={{ fontSize: 13, color: PT.primary, textDecoration: 'none', fontWeight: 600 }}>
            Lupa password?
          </a>
        </div>

        {error && (
          <p style={{ background: 'oklch(0.95 0.05 25)', color: 'oklch(0.45 0.18 25)', borderRadius: 8, padding: '8px 12px', fontSize: 13, margin: 0 }}>
            {error}
          </p>
        )}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        {(
          [
            { id: 'pharmacy' as SignupRole, icon: '🏥', label: 'Apotek', desc: 'Daftarkan apotek Anda' },
            { id: 'driver' as SignupRole, icon: '🚴', label: 'Driver', desc: 'Bergabung sebagai mitra pengemudi' },
          ]
        ).map((r) => (
          <button
            key={r.id}
      ...
      {/* Admin info box */}
      <div style={{ padding: '10px 14px', background: PT.bg, borderRadius: 10, border: `1px solid ${PT.border}`, fontSize: 12.5, color: PT.muted, lineHeight: 1.6 }}>
        🛡️ <strong style={{ color: PT.text }}>Admin?</strong> Akun admin dibuat melalui undangan langsung oleh Super Admin. Hubungi{' '}
        <a href="mailto:admin@pharmatrack.id" style={{ color: PT.primary, fontWeight: 600, textDecoration: 'none' }}>
...
export function PrimaryButton({
  ...
  return (
    <button
      ...
      onMouseEnter={(e) => { if (!pending) e.currentTarget.style.background = PT.primaryHover; }}
      onMouseLeave={(e) => { e.currentTarget.style.background = PT.primary; }}
    >
      {children}
    </button>
  );
