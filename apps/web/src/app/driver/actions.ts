...
export async function toggleDriverOnlineAction(online: boolean): Promise<{ ok: boolean }> {
  const session = await getSession();
  if (!session?.user || session.user.role !== 'driver') return { ok: false };
...
export type UpdateProfileResult = { ok: true } | { ok: false; reason: string };
